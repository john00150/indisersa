...
def get_page(browser, city, fh):
    ...
    for item in items:
        ...
        line = '"%s","%s","%s","%s","%s","%s","%s","%s","%s"\n' % (hotelname, hotelrating, hotelreview, hoteladdress, new_price, old_price, checkin, checkout, city)
        fh.write(line)
        #global conn, cur, max_id
...
def get_city(browser, city, choose_date, fh):
    ...
    while get_page(browser, city, fh):
        pass
...
def main():
    ...
    fh = open('hotels.csv', 'w')

    get_city(browser, 'Guatemla City, Guatemala', True, fh)
    get_city(browser, 'Antigua Guatemala, Guatemala', False, fh)
